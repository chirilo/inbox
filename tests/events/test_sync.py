...
def test_handle_changes(db, new_account):
    ...
    event_sync.provider.get_calendars = calendar_response
    event_sync.provider.get_events = event_response
    event_sync.sync()
    ...
    event_sync.provider.get_calendars = calendar_response_with_update
    event_sync.provider.get_events = event_response
    event_sync.sync()
    ...
    event_sync.provider.get_events = event_response_with_update
    event_sync.sync()
    ...
    event_sync.provider.get_events = event_response_with_delete
    event_sync.sync()
    ...
    event_public_ids = [id_ for id_, in db.session.query(Event.public_id).
                        filter(Event.namespace_id == namespace_id,
                               Event.calendar_id == first_calendar.id)]
    event_sync.provider.get_calendars = calendar_response_with_delete
    event_sync.sync()
    assert db.session.query(Calendar).filter(
        Calendar.namespace_id == namespace_id,
