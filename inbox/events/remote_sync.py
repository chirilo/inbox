...
class EventSync(BaseSyncMonitor):
    ...
    def sync(self):
        ...
        deleted_uids, calendar_changes = self.provider.get_calendars()
        with session_scope() as db_session:
        ...
        for (uid, id_) in calendar_uids_and_ids:
            deleted_uids, event_changes = self.provider.get_events(
...
def handle_calendar_deletes(namespace_id, deleted_calendar_uids, db_session):
    for uid in deleted_calendar_uids:
        ...
        if local_calendar is not None:
            # STOPSHIP(emfree): cascade to events on calendar, ensuring that we
